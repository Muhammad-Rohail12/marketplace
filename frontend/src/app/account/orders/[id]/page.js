...
function OrderDetailContent() {
  ...
  const load = useCallback(() => orderService.getMine(id).then((res) => setOrder(res.data.order)).finally(() => setIsLoading(false)), [id]);
  useEffect(() => { load(); }, [load]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Card>
        <h2 className="mb-2 text-sm font-semibold uppercase text-neutral-500">Shipping To</h2>
